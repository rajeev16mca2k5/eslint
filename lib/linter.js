 ...
 * @property {Map<string, Object>} parserMap The loaded parsers.
 ...
 * @param {Object[]} enabledEnvironments The environments enabled in configuration and with inline comments
 * @returns {Object} Resulting parser options after merge
 */
function resolveParserOptions(parserName, providedOptions, enabledEnvironments) {
    const parserOptionsFromEnv = enabledEnvironments
        .filter(env => env.parserOptions)
        .reduce((parserOptions, env) => lodash.merge(parserOptions, lodash.cloneDeep(env.parserOptions)), {});
 ...
 * @param {Map<string, Object>} ruleMapper A map from rule IDs to defined rules
...
function getEnv(slots, envId) {
    return (
        ...
        (BuiltInEnvironments.hasOwnProperty(envId) && BuiltInEnvironments[envId]) || // eslint-disable-line no-prototype-builtins
        null
    );
}
...
class Linter {
    ...
    _verifyWithoutProcessors(textOrSourceCode, providedConfig, filenameOrOptions) {
        ...
        let parserName = config.parser || DEFAULT_PARSER_NAME;

        if (typeof parserName === "object" && parserName !== null) {
            parserName = parserName.filePath;
        }
