...
const builtInRuleMap = new Map(Object.entries(BuiltInRules));
 ...
 * @param {function(envId:string): Object} [envMapper] A map from strings to loaded environments.
 * @returns {void}
 */
function validateEnvironment(
    environment,
    source,
    envMapper = id => builtInEnvMap.get(id)
) {
 ...
 * @param {string} source The name of the configuration source to report in any errors.
 * @param {function(ruleId:string): Object} ruleMapper A map from strings to loaded rules
 * @returns {void}
 */
function validateRules(
    rulesConfig,
    source,
    ruleMapper = id => builtInRuleMap.get(id)
) {
    if (!rulesConfig) {
        return;
    }
 ...
 * @param {string} source The name of the configuration source to report in any errors.
 * @param {function(ruleId:string): Object} [ruleMapper] A map from strings to loaded rules.
 * @param {function(envId:string): Object} [envMapper] A map from strings to loaded envs.
 * @returns {void}
 */
function validate(config, source, ruleMapper, envMapper) {
...
function validateConfigArray(configArray) {
    ...
    /**
     * Get an environment definition.
     * @param {string} id The environment ID to get.
     * @returns {Object|null} The environment definition.
     */
    function getEnv(id) {
        return (
            configArray.pluginEnvironments.get(id) ||
            builtInEnvMap.get(id) ||
            null
        );
    }

    /**
     * Get an rule definition.
     * @param {string} id The rule ID to get.
     * @returns {Object|null} The rule definition.
     */
    function getRule(id) {
        return (
            configArray.pluginRules.get(id) ||
            builtInRuleMap.get(id) ||
            null
        );
    }
    ...
    for (const element of configArray) {
        ...
        validateEnvironment(element.env, element.name, getEnv);
        validateRules(element.rules, element.name, getRule);
    }
}
